...
def detect_recognize_face_text_cccd(image_crop_front, image_crop_back, card_side_predict, model_detect_text, model_recognition):
    image_front = image_crop_front.copy()
    image_back = image_crop_back.copy()

    confidence_list = {}
    result = {}
    result["identCardType"] = ""
    ...
    # type_texts = ["Loaithe", "Maso", "Hoten", "Namsinh", "Gioitinh", "Quoctich", "Quequan", "Noithuongtru", "Anh", "HSD"]
    type_texts = ["Maso", "Hoten", "Namsinh", "Gioitinh", "Quoctich", "Quequan", "Noithuongtru", "HSD", "Loaithe"]
    ...
    # Loai the, Quoc tich
    id_card_type = result_text[8][0]
    if "CĂN" in id_card_type or "CƯỚC" in id_card_type or "CAN CUOC" in id_card_type or "CĂN CƯỚC" in id_card_type:
        result['identCardType'] = "CĂN CƯỚC CÔNG DÂN"
        result['identCardIssuePlace'] = "CỤC TRƯỞNG CỤC CẢNH SÁT ĐKQL CƯ TRÚ VÀ DLQG VỀ DÂN SỰ"
    else:
        ...
        result['identCardIssuePlace'] = "CỤC TRƯỞNG CỤC CẢNH SÁT ĐKQL CƯ TRÚ VÀ DLQG VỀ DÂN SỰ"
        # Quoc tich
        id_card_ethnic = result_text[4][0]
        result['identCardEthnic'] = id_card_ethnic
        confidence_list['identCardEthnic'] = result_confidences[4][0]
    confidence_list['identCardType'] = result_confidences[8][0]
    # Ma So
    ...
    text_nguyenquan = text_nguyenquan.replace("Trung Giang, Gio Linh, Quảng Trị Nam", "Trung Giang, Gio Linh, Quảng Trị")
